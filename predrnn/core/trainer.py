...
valid_dict['input_raw_data'] = np.zeros(shape=(LENGTH, N_CHANNELS, INPUT_HEIGHT, INPUT_WIDTH), dtype=np.int32)
...
for i in range(LENGTH):
    ...
    for k in range(TOTAL_LENGTH):
        valid_dict['input_raw_data'][i+k] = np.reshape(valid_data[i+k], (N_CHANNELS, INPUT_HEIGHT, INPUT_WIDTH))
...
for i in range(LENGTH):
    ...
    for k in range(TOTAL_LENGTH):
        test_dict['input_raw_data'][i+k] = np.reshape(test_data[i+k], (N_CHANNELS, INPUT_HEIGHT, INPUT_WIDTH))
